...
```{r ans22}
target_regex <- context_picked
A_regex <- "\\s{2}"

regex_pattern <- glue::glue("(?<={A_regex})[:graph:]")


print(regex_pattern)


context_picked_split<-str_split(context_picked,regex_pattern )
...
```{r ans23}
mention_duan<-
str_extract_all(context_picked_split[[1]][1:214],"段(譽|兄|兄弟|大哥|相公|君)","姓段的小子")
