...
[臺南市運動地圖](http://data.gov.tw/dataset/6217):
```{r}
library(jsonlite)
...
```{r}
get2 <-listExample["學期"] #listExample只留下學期元素，是個只有一個元素的list
...
c=gender[c(1,2,5)]
econOnly=list(
  majors=a,names=b,gender=c
)
econOnly[c("names","gender")]
econFemaleOnly=econOnly[["gender"]][c(T,T,F)]
econOnly[["names"]][c(1,2)]
```
