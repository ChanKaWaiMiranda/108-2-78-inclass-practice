...
data_selected <- 
  list(
    name=example$name[pick_`小新`],
    weight=example$weight[pick_`小新`],
    height=example$height[pick_`小新`],
    hobby=example$hobby[pick_`小新`],
    residence=example$residence[pick_`小新`],
    allowance=example$allowance[pick_`小新`],
    bloodType=example$bloodType[pick_`小新`]
  )
```
...
由於`.x[pick_小新]`是個公式化的動作，

在R，`~`代表要形成一個公式(formula)，
...
[舞蹈表演資訊](https://data.gov.tw/dataset/6015)
...
[一般天氣預報-今明36小時天氣預報](https://opendata.cwb.gov.tw/opendatadoc/MFC/ForecastElement.pdf)
```{r}
...
氣象局針對每個縣市提供了不同氣象因子在未來三個時段的預報。每個都市的氣象因子預報有那些，以第一個都市為例，它存在
```{r}
...
```
```{r}
sort(threeHeigths,deacreasing=T) ->sorted_heights
names(sorted_heights)->sorted_persons
sorted_persons[[1]]->highestPerson
```
