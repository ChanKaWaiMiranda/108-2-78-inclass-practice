...
```{r ans1}
levels_numeral<-levels(numerals_Gujarati)
as.factor(numerals_Gujarati)
as.integer(as.factor(numerals_Gujarati))
...
```{r ans23}
poem_song$paragraphs[[22]]<- "斗柄仙城曉，龍鱗下隰通。 三竿眺羲馭，十里過商中。 水藉天容碧，塵依驛轍紅。 喜聞何力語，應爲樂秋豐。"

poem_song$paragraphs[[22]]
```
## 3 Dcard
...
```{r ans33}
post_time<-c(post$comments$Datetime)
DateTime<-ymd_hms(post_time,tz="Asia/Taipei")
class(DateTime)
```
...
```{r ans34}
library(lubridate)
Poland_time<-ymd_hms(DateTime)
Poland_time1<-with_tz(Poland_time,tz="Asia/Taipei")
post[[""]]
