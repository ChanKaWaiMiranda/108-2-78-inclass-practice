...
***
[嚴重特殊傳染性肺炎](http://data.gov.tw/dataset/120711)
...
```{r,eval=T,echo=F,results="asis",fig.cap="Sorting named vectors"}
img_centuring(src="img/namedVector.svg",width="60%")
```
...
因為`The output type is determined from the highest type of the components in the hierarchy NULL < raw < logical < integer < double < complex < character < list < expression.` 
factor
是integer type所以會被當integer後才串接在一起。
...
```{r}
str(garbageRoutes)
route_regex <- "[:graph:]+路線"
route <- str_extract(garbageRoutes$linename, route_regex)
factor(route) -> garbageRoutes$route
garbageRoutes$route
```

2. 請設定garbageRoutes$route的levels依其路線停靠總站數由小排到大陳列。
```{r}
stop_numbers <- table(garbageRoutes$route)
stop_numbers <- sort(stop_numbers, decreasing = T)
new_level <- names(stop_numbers)
garbageRoutes$route <- factor(garbageRoutes$route, levels = new_level)
